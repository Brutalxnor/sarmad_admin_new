...
async function inspect() {
    ...
    console.log(`Inspecting assessments for user ${userId}...`);
    const { data, error } = await supabase.from("assessment").select("*").eq("user_id", userId);

    if (error) {
        console.error("Error:", error.message);
    } else {
        console.log("Success! Assessments found:", data.length);
        data.forEach((a, i) => {
            console.log(`Assessment ${i + 1}:`, a);
        });
    }
}
