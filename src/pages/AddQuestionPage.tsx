...
export default function AddQuestionPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            await createQuestion.mutateAsync({
                ...
                category: category,
                assess_version: decodedVersion as unknown as number, // Cast to unknown then number to satisfy types if needed
    ...
    return (
        <div className="max-w-[1200px] mx-auto px-6 py-6 bg-[#F9FBFC] dark:bg-slate-900 min-h-screen animate-fade-in transition-colors duration-300" dir={direction}>
            ...
            <div className={`flex justify-between items-start mb-10 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                <div className={`flex items-center gap-4 text-slate-400 font-bold text-sm ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                    ...
                    <span className="text-slate-800 dark:text-slate-100 text-base">{language === 'ar' ? 'إضافة سؤال جديد' : 'Add New Question'}</span>
                </div>

                <div className="flex items-center gap-4">
                    <button
                        onClick={() => navigate(`/assessments/${version}/questions`)}
                        className="px-12 py-4 rounded-[1.25rem] bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 text-slate-400 dark:text-slate-300 font-black hover:bg-gray-50 dark:hover:bg-slate-700 transition-all shadow-sm"
                    >
            ...
            <div className={`flex items-center gap-4 mb-12 ${isRTL ? 'justify-start' : 'justify-end'}`}>
                <div className={`flex items-center gap-3 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                    ...
                    <span className="bg-sky-50 dark:bg-sky-500/10 text-sky-500 dark:text-sky-400 px-3 py-1 rounded-lg text-sm font-black">
                        {decodedVersion}
                    </span>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto mb-10 bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-sm animate-fade-in relative overflow-hidden transition-colors duration-300">
                {/* Decorative background element */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 dark:bg-slate-700/50 rounded-full blur-3xl -z-0 opacity-50 transform translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>

                <div className={`flex items-center gap-4 mb-8 relative z-10 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                    <div className="w-12 h-12 rounded-2xl border-2 border-[#1EAD93]/20 bg-[#1EAD93]/5 text-[#1EAD93] dark:text-[#2DD4BF] flex items-center justify-center shrink-0">
                        <Info size={24} />
                    </div>
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                        <h3 className="text-2xl font-black text-[#1E293B] dark:text-slate-100 mb-1">
                ...
                <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                    {/* Card 1 (75% - 100%) */}
                    <div className="bg-white dark:bg-slate-800/80 border-2 border-[#F3E8FF] dark:border-purple-500/20 rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all hover:shadow-md hover:border-[#D8B4FE] dark:hover:border-purple-500/40">
                        <div className="bg-[#FAF5FF] dark:bg-purple-500/10 text-[#7E22CE] dark:text-purple-400 font-black px-6 py-2.5 rounded-2xl text-sm mb-4 w-full tracking-wider">
                            100% - 75%
                        </div>
                        <div className="font-black text-[#4C1D95] dark:text-purple-300 text-lg mb-2">
                            {language === 'ar' ? 'أنماط الأرق' : 'Insomnia Patterns'}
                        </div>
                        <div className="text-xs font-bold text-[#9333EA] dark:text-purple-400">
                            {language === 'ar' ? 'توصية ببرنامج CBT-I' : 'CBT-I Program Recommendation'}
                        </div>
                    </div>

                    {/* Card 2 (25% - 75%) */}
                    <div className="bg-white dark:bg-slate-800/80 border-2 border-[#D1FAE5] dark:border-emerald-500/20 rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all hover:shadow-md hover:border-[#6EE7B7] dark:hover:border-emerald-500/40">
                        <div className="bg-[#ECFDF5] dark:bg-emerald-500/10 text-[#047857] dark:text-emerald-400 font-black px-6 py-2.5 rounded-2xl text-sm mb-4 w-full tracking-wider">
                            75% - 25%
                        </div>
                        <div className="font-black text-[#065F46] dark:text-emerald-300 text-lg mb-2">
                    ...
                    {/* Card 3 (< 25%) */}
                    <div className="bg-white dark:bg-slate-800/80 border-2 border-[#F1F5F9] dark:border-slate-600 rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-all hover:shadow-md hover:border-[#CBD5E1] dark:hover:border-slate-500">
                        <div className="bg-[#F8FAFC] dark:bg-slate-700/50 text-[#334155] dark:text-slate-300 font-black px-6 py-2.5 rounded-2xl text-sm mb-4 w-full tracking-wider">
                            {'< 25%'}
                        </div>
                        <div className="font-black text-[#1E293B] dark:text-slate-200 text-lg mb-2">
            ...
            <div className="space-y-10 max-w-4xl mx-auto">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <textarea
                                ...
                                className="w-full min-h-[180px] p-8 rounded-[2.5rem] bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-sm outline-none focus:ring-2 focus:ring-[#0095D9]/10 focus:border-[#0095D9]/20 transition-all resize-none font-bold text-slate-800 dark:text-slate-100 text-lg"
                            />
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-[#F4F9FB]/50 dark:bg-slate-800/50 rounded-[2.5rem] p-8 border border-sky-100/30 dark:border-slate-700 space-y-4 min-h-[100px] flex flex-col justify-center">
                        {answers.length === 0 ? (
                            ...
                            answers.map((answer, index) => (
                                <div
                                    key={answer.id}
                                    className={`bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm flex items-center gap-6 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}
                                >
                                    {/* Option Number */}
                                    <div className="w-10 h-10 rounded-full border-2 border-slate-100 dark:border-slate-600 flex items-center justify-center text-slate-400 dark:text-slate-500 font-black text-sm shrink-0">
                                        {index + 1}
                                    </div>
                                    ...
                                    <div className="flex-1">
                                        <input
                                            ...
                                            className="w-full bg-transparent outline-none font-black text-slate-700 dark:text-slate-100 placeholder:text-slate-200 dark:placeholder:text-slate-600"
                                        />
                                    </div>

                                    {/* Points Input */}
                                    <div className={`flex items-center gap-4 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
                                        <span className="text-slate-800 dark:text-slate-300 font-black text-sm whitespace-nowrap">
                                        ...
                                        <input
                                            ...
                                            className="w-20 bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 rounded-xl px-4 py-2 text-center font-black text-[#35788D] dark:text-[#4AA0BA] focus:ring-2 focus:ring-[#35788D]/10 outline-none transition-all"
                                        />
                                    </div>
