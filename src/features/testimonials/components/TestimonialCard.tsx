...
export function TestimonialCard({ testimonial, onDelete, onToggleActive }: TestimonialCardProps) {
    ...
    return (
        <div className={`group bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 flex flex-col h-full transform hover:-translate-y-1 ${!testimonial.is_active ? 'opacity-75 grayscale-[0.3]' : ''}`}>
            ...
            <div className="relative aspect-[4/5] w-full overflow-hidden bg-slate-100">
                {testimonial.image_url ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50 text-slate-200">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest">{isRTL ? 'بدون صورة' : 'No Image'}</span>
                    </div>
                )}
                ...
                <div className={`absolute top-6 left-6 z-20`}>
                    <div
                        ...
                        className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-all duration-300 ${testimonial.is_active ? 'bg-emerald-500 shadow-lg shadow-emerald-500/40' : 'bg-slate-400/50 backdrop-blur-md'}`}
                    >
            ...
            <div className={`p-8 flex flex-col flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                <h3 className="text-xl font-black text-slate-800 mb-1 group-hover:text-[#35788D] transition-colors line-clamp-1">
                    {name}
                </h3>
                {role && (
                    <p className="text-[10px] font-black text-[#0095D9] uppercase tracking-widest mb-4 opacity-80">
                        {role}
                    </p>
                )}

                <p className="text-slate-500 font-bold text-sm leading-relaxed line-clamp-4 mb-6 italic">
                    {content}
                </p>
