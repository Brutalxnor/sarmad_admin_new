        ...
        >
            <div className={`space-y-8 py-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                <div className="space-y-3">
                    ...
                    <p className="text-xl font-black text-slate-800 bg-[#F4F9FB] p-6 rounded-[1.5rem] border border-slate-100/50 shadow-inner leading-relaxed">
                        {question.question}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <div className="inline-flex items-center px-4 py-2 rounded-xl text-sm font-black bg-[#E0F2F7] text-[#35788D] border border-[#35788D]/10">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        {question.answers?.map((ans, idx) => (
                            <div
                                key={ans.id || idx}
                                className={`flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-[#35788D]/20 hover:shadow-sm transition-all group ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}
                            >
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-xl bg-[#F4F9FB] flex items-center justify-center text-[#35788D] font-black text-sm">
                                        {idx + 1}
                                    </div>
                                    <span className="text-slate-700 font-bold text-lg">{ans.answer}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="h-2 w-24 bg-slate-100 rounded-full overflow-hidden hidden sm:block">
                                        <div
                                            className="h-full bg-[#0095D9] transition-all duration-1000"
                                            style={{ width: `${ans.percentage}%` }}
                                        />
                                    </div>
                                    <span className="bg-[#E0F2F7] text-[#35788D] px-3 py-1.5 rounded-xl text-sm font-black min-w-[3.5rem] text-center">
                                        {ans.percentage}%
                                    </span>
                                </div>
                            </div>
